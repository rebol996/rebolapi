...
const PATTERNS: Array<{ type: string; pattern: RegExp }> = [
  { type: "api_key", pattern: /\b(?:sk-|sk_live_|sk_test_|pk_live_|pk_test_)[a-zA-Z0-9]{20,}\b/g },
  { type: "private_key", pattern: /-----BEGIN (?:RSA |EC )?PRIVATE KEY-----/g },
  { type: "database_url", pattern: /\b(?:postgres|mysql|mongodb|redis):\/\/[^\s]+/g },
  { type: "token", pattern: /\b(?:ghp_|gho_|github_pat_|glpat-|xox[bpras]-)[a-zA-Z0-9]{20,}\b/g },
  ...
  { type: "credential", pattern: /(?:api_key|apikey|secret|access_token|auth_token)\s*[=:]\s*['"][^\s'"]{8,}['"]/gi },
];
